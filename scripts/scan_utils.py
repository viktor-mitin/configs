...
_reStructHeader = re.compile(r"(typedef\s+)?struct(\s+(\S+))?\s*\{(\d+)\{", re.M|re.DOTALL)
...
def parse_cpss_applicable_devices(comment,applicableDevices):
    ...
    if description.upper() in applicableDevices:
        return description,applicableDevices[description.upper()]
...
#
_reCommentedTypedef = re.compile(r"(/\*[^*]*\*+(?:[^/*][^*]*\*+)*/)\s*typedef\s+(enum|struct)\s+\{",\
           re.M | re.DOTALL)
